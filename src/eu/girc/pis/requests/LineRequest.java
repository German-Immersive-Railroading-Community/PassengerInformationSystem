package eu.girc.pis.requests;

import eu.girc.pis.html.LineHtml;
import eu.girc.pis.main.Main;
import eu.girc.pis.main.RequestHandler;
import io.undertow.server.HttpHandler;
import io.undertow.server.HttpServerExchange;

public class LineRequest implements HttpHandler{

	@Override
	public void handleRequest(HttpServerExchange exchange) throws Exception {
		String args[] = RequestHandler.getArgs(exchange.getRequestPath());
		if (args.length == 1) {
			RequestHandler.sendHtml(exchange, LineHtml.buildLineList(Main.getLines()));
		} else if (args.length == 2 && Main.getLines().contains(args[1])) {
			RequestHandler.sendHtml(exchange, LineHtml.buildLine(Main.getLines().get(args[1])));
		}
	}
	
}
